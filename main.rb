...
def get_unfetch_app
  ...
  @client["app_list"].insert_one({:_id => "com.sd.google.helloKittyCafe", :package => "com.sd.google.helloKittyCafe"})#in waiting state
end
...
def scrape(id)
  ...
  begin
    ...
    page.css(app_suggest_app_css_path).each do |el|
      ...
      if @client["app_list"].find({:_id => next_app}).count == 0 and not @apps_id.include?(next_app)# Not in queue
        @client["app_list"].insert_one({:_id => next_app, :package => next_app})#in waiting state
        @apps_id << next_app
      end
      puts next_app
    end
    ...
    puts record
    @client["app_info"].insert_one(record)
  rescue
    puts id
  end
